$(function(){
	...
	function makeDecoSqEl(){
		var $container = $(".title-deco-el-holder");
		var conWidth = Math.floor($container.width()*1.1);
		var conHeight = Math.floor($container.height()*6);

		var backgroundColorArr = ["#ffda3d", "#f39431","#ffc34e","#e1bd5c","#ffec9a"];	
		var getRandomColor = function(){
			var n = randomRange(0, backgroundColorArr.length);
			return backgroundColorArr[n];
		};

		for (var i=0; i <70; i++){
			var pixel = $("<div />", { "class": "deco-el" } );
			var x = randomRange(0, conWidth)+"px";
			var y = randomRange(0, conHeight)+"px";
			if(isMobile==true){
				var width = randomRange(8, 15)+"px";
			}else{
				var width = randomRange(10, 25)+"px";
			}
			
			pixel.css({"width": width, "height": width});
			pixel.css({"left": x});
			pixel.css({"top": y});
			pixel.css({"background": getRandomColor()});

			pixel.css({"transform": "scale(0.1)"});
	...
	if(isMobile==true){
		...
		$("#S01_02").find("img").attr("src", "http://img.khan.co.kr/spko/storytelling/2021/publicdesign/wando/s02-01-m.jpg");
		$("#S03_01").find("img").attr("src", "http://img.khan.co.kr/spko/storytelling/2021/publicdesign/wando/s02-vill-full-3-m.jpg");
		$("#S05_04").find("img").attr("src", "http://img.khan.co.kr/spko/storytelling/2021/publicdesign/wando/shelter_art_m.jpg");
	}else{
		$(".title-svg-m").html("");
		if(ieTest==true){
    ...
    function avoid100vh(){
		...
		$(".story-header-with-video .video-fullScreen video").height(screenHeight*1.2);
		$(".story-title").css({"top":screenHeight*0.3+"px"});
	}
	...
	$(window).scroll(function(){
		...
		if(nowScroll > screenHeight*0.2){
			$(".going-down").stop().animate({"opacity":"0"});
		}else{
			$(".going-down").stop().animate({"opacity":"1"});
		}
		...
		if(introPathAniDone ==false && (nowScroll+screenHeight) >=$(".story-title").offset().top ){
			introPathAniDone = true;
			activTitlePathAni();
		}
		...
		if(nowScroll > $(".tt-slider-list").offset().top && twActiveDone ==false){
			activataTw();
			twActiveDone = true;
		}
	
		$(".deco-el").each(function(i){
		...
		$(".story-header-with-video").each(function(i){
			if( nowScroll > screenHeight*7){
				$(this).find("video").get(0).pause();
			}else{
				$(this).find("video").get(0).play();
			}
